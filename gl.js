...
void main(){
  ...
  sum += toLin(texture2D(uTex, vUv - o2).rgb) * 0.12; w += 0.12;
  sum += toLin(texture2D(uTex, vUv + o3).rgb) * 0.10; w += 0.10;
  sum += toLin(texture2D(uTex, vUv - o3).rgb) * 0.10; w += 0.10;
  ...
  wideSum += toLin(texture2D(uTex, vUv + wOffT).rgb);
  wideSum += toLin(texture2D(uTex, vUv - wOffT).rgb);
  wideSum += toLin(texture2D(uTex, vUv + wOffR).rgb);
  wideSum += toLin(texture2D(uTex, vUv - wOffR).rgb);
  wideSum *= 0.25;
  ...
  gl_FragColor = vec4(toSRGB(glow), 1.0);
}
`;
